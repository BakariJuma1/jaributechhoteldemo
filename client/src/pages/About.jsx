...
const values = [
  {
    title: 'Fire & Craft',
    description: 'Every cut of meat is slow-smoked over real hardwood for 12 to 18 hours. No shortcuts, no compromises.',
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 18.657A8 8 0 016.343 7.343S7 9 9 10c0-2 .5-5 2.6-7C14 5 15.927 7.655 15.927 7.655S17 6 18 6c0 4-2 6-4 8 1 0 2.5-.5 3.657-1.343z" /></svg>),
  },
  {
    title: 'Locally Sourced',
    description: 'We partner with Kenyan farmers and suppliers to bring the freshest, most honest ingredients to your plate.',
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>),
  },
  {
    title: 'Community First',
    description: 'Jiko House was born in Nairobi, for Nairobi. We give back to the neighborhoods that make us who we are.',
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" /></svg>),
  },
  {
    title: 'Always Welcoming',
    description: 'Whether you are in for a quick lunch or a long evening with family, every guest leaves feeling at home.',
    icon: (<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>),
  },
]

const team = [
  { name: 'Amani Njoroge', role: 'Head Pitmaster', bio: '20 years behind the fire. Amani trained in Texas before bringing his mastery back to Nairobi.', image: 'https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=400&q=80' },
  { name: 'Zawadi Ochieng', role: 'Executive Chef', bio: 'A graduate of Le Cordon Bleu, Zawadi fuses classical French technique with bold East African flavours.', image: 'https://images.unsplash.com/photo-1567532939604-b6b5b0db2604?w=400&q=80' },
  { name: 'Baraka Mutua', role: 'General Manager', bio: 'Baraka ensures every guest experience at Jiko House exceeds expectations from the moment they walk in.', image: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400&q=80' },
]
      ...
      <section className="bg-white py-10 md:py-20 px-4">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-14 items-center">
          ...
          <div className="rounded-2xl overflow-hidden shadow-lg aspect-[4/3]">
            <img src="https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=800&q=80" alt="Nairobi street food scene" className="w-full h-full object-cover" loading="lazy" />
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#F4F4F2] py-10 md:py-20 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
            {values.map((v) => (
              <div key={v.title} className="bg-white rounded-2xl p-4 md:p-7 shadow-sm">
                <div className="w-10 h-10 rounded-full bg-[#D89B3F]/10 text-[#D89B3F] flex items-center justify-center mb-3">
                  {v.icon}
                </div>
                <h3 className="font-serif text-base md:text-xl font-bold text-[#2B2D3A] mb-1">{v.title}</h3>
                <p className="font-sans text-gray-500 text-xs md:text-sm leading-relaxed">{v.description}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-[#2B2D3A] py-10 md:py-20 px-4">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="space-y-5 md:space-y-8">
            {milestones.map((m, i) => (
              <div key={m.year} className={`relative flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4 ${i % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                <div className={`md:w-1/2 ${i % 2 === 0 ? 'md:text-right md:pr-10' : 'md:text-left md:pl-10'}`}>
                  <span className="font-serif text-2xl md:text-3xl font-bold text-[#D89B3F]">{m.year}</span>
                  <h3 className="font-serif text-base md:text-xl font-bold text-white mt-0.5">{m.label}</h3>
                  <p className="font-sans text-white/60 text-xs md:text-sm mt-1 leading-relaxed">{m.detail}</p>
                </div>
                <div className="hidden md:flex absolute left-1/2 -translate-x-1/2 w-3 h-3 rounded-full bg-[#D89B3F] border-4 border-[#2B2D3A]" />
          ...
          <div className="grid grid-cols-3 gap-4 md:gap-8">
            {team.map((member) => (
              <div key={member.name} className="text-center group">
                ...
                <h3 className="font-serif text-sm md:text-xl font-bold text-[#2B2D3A]">{member.name}</h3>
                <p className="font-sans text-[#D89B3F] text-xs md:text-sm font-semibold mt-0.5 mb-1">{member.role}</p>
                <p className="hidden md:block font-sans text-gray-500 text-sm leading-relaxed max-w-xs mx-auto">{member.bio}</p>
              </div>
            ))}
          </div>
        </div>
