    ...
    <main className="pt-16">
      ...
      <section className="bg-[#F4F4F2] py-5 md:py-16 px-4">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-10">
          ...
          <div className="space-y-4">
            <div className="bg-white rounded-2xl p-4 md:p-7 shadow-sm">
              ...
              <div className="space-y-3">
                <ContactRow icon="pin" label="Address" value={config.address || siteConfig.address} />
                <ContactRow
                  icon="phone" label="Phone"
                  value={
                    <a href={`tel:${(config.phone || siteConfig.phone).replace(/\s/g, '')}`} className="text-[#D89B3F] hover:underline">
                ...
                <ContactRow
                  ...
                  value={
                    <>
                      <span className="block">Mon-Fri: {config.hours?.weekdays || siteConfig.hours.weekdays}</span>
                      <span className="block">Sat-Sun: {config.hours?.weekends || siteConfig.hours.weekends}</span>
                    </>
                  }
                />
              </div>
          ...
          {/* Right: form → opens WhatsApp on submit */}
          <div className="bg-white rounded-2xl p-4 md:p-7 shadow-sm">
            <h2 className="font-serif text-lg md:text-2xl font-bold text-[#2B2D3A] mb-1">Send a Message</h2>
            <p className="font-sans text-gray-400 text-sm mb-3">Fill in your details and we'll open a WhatsApp chat for you.</p>
            <form onSubmit={handleSubmit} className="space-y-2.5 md:space-y-4">
              ...
              <button type="submit" className="btn-primary w-full py-2.5 md:py-3 flex items-center justify-center gap-2">
      ...
      <section className="bg-white py-5 md:py-14 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-5">
            ...
            <Divider />
            <p className="font-sans text-gray-500 text-sm mt-1">{config.address || siteConfig.address}</p>
          </div>
          <div className="rounded-2xl overflow-hidden shadow-md h-56 md:h-96">
            <iframe
              src={config.mapEmbedUrl || siteConfig.mapsEmbed}
